...
var Ui = new Class({
	...
	_drawDevice: function(id) {
		...
		var sIn = new Element('span', {'class': 'state'}).inject(s);
		var s = new Element('span', {'class': 'direction', 'html': 'Out'}).inject(p);
		var sOut = new Element('span', {'class': 'state'}).inject(s)
		this._devices[id].listeners.osc = {
			'in': new Fx.Morph(sIn),
			'out': new Fx.Morph(sOut)
		};
		...
		var s = new Element('span', {'class': 'direction', 'html': 'Out'}).inject(p);
		var sOut = new Element('span', {'class': 'state'}).inject(s)
		this._devices[id].listeners.midi = {
			'in': new Fx.morph(sIn),
			'out': new Fx.morph(sOut)
		};
	...
	_eventAddDevice: function(e) {
		...
		emulator.start();
		this._devices[emulator._attributes.id] = {'device': emulator, 'listeners': {}};
