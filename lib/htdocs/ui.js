...
var Ui = new Class({
	...
	initialize: function(options) {
		...
		this._form = new Element('form', {'method': 'post'}).inject(this._container);
		this._setMidiPorts()._drawForm()._setEvents();
	...
	_drawForm: function() {
		...
		this._drawOptionInput('deviceSerial', 'Serial Number', alias)
			...
			new Element('button', {'type': 'button', 'class': 'add', 'html': 'Add device'}).inject(this._form);
			new Element('br').inject(this._form);
			new Element('input', {'type': 'file', 'class': 'load', 'html': 'Load config', 'name': 'configLoad'}).inject(this._form);
